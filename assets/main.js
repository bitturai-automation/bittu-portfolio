...
(function(){
  ...
  function renderProjects(){
    ...
    grid.innerHTML = items.map((p,i)=>{
      ...
      return `<div class="project-card ${sizeClass}" data-index="${D.projects.indexOf(p)}" data-cursor="VIEW">${media}${caption}</div>`;
    }).join('');

    grid.querySelectorAll('.project-card').forEach(card=>{
      card.addEventListener('click', ()=> openProject(parseInt(card.dataset.index,10)));
    });
  ...
  function openProject(index){
    ...
    const rows = [
      ['Category', categoryLabel],['Subcategory', p.subcategory],['Year', p.year],
      ['Client', p.client],['Role', p.role],['Tools', (p.tools||[]).join(', ')]
    ].filter(r=>r[1]);
    const images = p.images || [];

    let mediaHtml;
    if (p.video){
      mediaHtml = `<video src="${p.video}" controls playsinline></video>`;
    } else if (images.length){
      mediaHtml = `
        <img id="modal-main-img" src="${images[0]}" alt="${p.title||''}">
        ${images.length > 1 ? `<div class="modal-thumbs">${images.map((src,i)=>`<img src="${src}" data-src="${src}" class="${i===0?'active':''}">`).join('')}</div>` : ''}`;
    } else {
      mediaHtml = `<div class="project-video-tile" style="aspect-ratio:4/5;"><small>${p.title||''}</small></div>`;
    }

    document.getElementById('modal-content').innerHTML = `
      <div class="modal-media" data-lenis-prevent>${mediaHtml}</div>
      <div class="modal-info" data-lenis-prevent>
        <h2 class="big-heading" style="font-size:clamp(28px,4vw,46px);">${p.title||''}</h2>
        <div style="display:flex;flex-wrap:wrap;gap:24px;margin:24px 0 32px;">
          ${rows.map(r=>`<div><div style="font-size:11px;text-transform:uppercase;letter-spacing:.05em;opacity:.5;">${r[0]}</div><div style="font-family:var(--display);font-weight:600;">${r[1]}</div></div>`).join('')}
        </div>
        ${p.description ? `<p style="font-size:16px;line-height:1.6;">${p.description}</p>` : ''}
        ${p.challenge ? `<h3 class="h-eyebrow">Challenge</h3><p style="line-height:1.6;">${p.challenge}</p>` : ''}
        ${p.approach ? `<h3 class="h-eyebrow">Approach</h3><p style="line-height:1.6;">${p.approach}</p>` : ''}
        ${p.outcome ? `<h3 class="h-eyebrow">Overview</h3><p style="line-height:1.6;">${p.outcome}</p>` : ''}
        ${p.link ? `<p style="margin-top:24px;"><a href="${p.link}" target="_blank" rel="noopener" style="text-decoration:underline;">View live / full case study ↗</a></p>` : ''}
      </div>`;
    ...
    if (mainImg){
      mainImg.addEventListener('click', ()=> openLightbox(mainImg.src, p.title||''));
    }
  ...
  function renderContact(){
    document.getElementById('btn-whatsapp').href = D.person.whatsapp;
    document.getElementById('btn-email').href = `mailto:${D.person.email}`;
    document.getElementById('btn-call').href = D.person.phoneHref;
    document.getElementById('contact-direct').innerHTML =
      `${D.person.phone} &nbsp;·&nbsp; <a href="mailto:${D.person.email}">${D.person.email}</a> &nbsp;·&nbsp; <a href="https://${D.person.portfolio}" target="_blank" rel="noopener">${D.person.portfolio}</a>`;
  }
  ...
  /* ---------------- LOADER ----------------
     A visitor (recruiter or otherwise) can skip the intro at any time by
     clicking/tapping the loader or pressing any key — the count/animation
     is a nice first impression, not something anyone should be stuck
     waiting through on a second look or a slow moment. */
  function runLoader(done){
    ...
    function finish(instant){
      if (finished) return;
      finished = true;
      const left = loader.querySelector('.loader-half.left');
      const right = loader.querySelector('.loader-half.right');
      const video = document.getElementById('loader-video');
      const overlay = loader.querySelector('.loader-video-overlay');
      if (window.gsap && !reduced && !instant){
        const tl = gsap.timeline({onComplete:()=>{ loader.remove(); document.body.classList.remove('loading'); done(); }})
          .to(left, {yPercent:-100, duration:.7, ease:'power4.inOut'})
          .to(right, {yPercent:100, duration:.7, ease:'power4.inOut'}, '<');
        const fadeTargets = [video, overlay].filter(Boolean);
        if (fadeTargets.length) tl.to(fadeTargets, {opacity:0, duration:.5, ease:'power2.out'}, '<0.15');
      } else {
        loader.remove(); document.body.classList.remove('loading'); done();
      }
    }
  ...
  function initCursor(){
    ...
    document.body.addEventListener('mouseout', e=>{
      if (!e.relatedTarget || !e.relatedTarget.closest || !e.relatedTarget.closest('[data-cursor], .project-card, .cert-flip, img, .btn, button, .big-btn')){
        cursor.classList.remove('big'); label.textContent='';
      }
    });

    // magnetic buttons
    document.querySelectorAll('.btn, .big-btn').forEach(btn=>{
  ...
  function initHero(){
    ...
    const l1 = document.getElementById('hero-line-1');
    const l2 = document.getElementById('hero-line-2');

    if (reduced){
      gsap.set([l1,l2], { xPercent:0, x:0, y:0, rotate:0, scale:1, opacity:1 });
    ...
    ScrollTrigger.matchMedia({
      ...
      /* Desktop / tablet-landscape: original left/right converging composition — unchanged. */
      "(min-width:861px)": function(){
        gsap.set(l1, { xPercent:-8 });
        gsap.set(l2, { xPercent:8 });

        const tl = gsap.timeline({
          scrollTrigger:{ trigger:'#hero', start:'top top', end:'bottom top', scrub:0.6 }
        });

        tl.to(l1, { xPercent:36, rotate:-2, scale:0.92, ease:'none' }, 0)
          .to(l2, { xPercent:-36, rotate:2, scale:1.05, ease:'none' }, 0)
          .to(l1, { xPercent:60, rotate:0, scale:1, textAlign:'center', ease:'none' }, 0.5)
          .to(l2, { xPercent:-60, rotate:0, scale:1, ease:'none' }, 0.5)
          .to('.hero-meta', { opacity:0, ease:'none' }, 0.55)
          .to('.hero-shape', { opacity:0, scale:0.4, ease:'none' }, 0.5)
          .to(cta, { opacity:1, y:0, duration:0.001 }, 0.85);
      },

      /* Mobile: the desktop composition relies on wide-viewport horizontal travel, which
         has no room to work with on a narrow screen and was pushing the headline off the
         left edge. Small screens instead get a simple entrance animation that plays
         immediately on load — centered, no horizontal movement, so it can never clip. */
      "(max-width:860px)": function(){
        gsap.set([l1,l2], { xPercent:0, x:0, y:26, opacity:0, rotate:0, scale:1 });

        const tl = gsap.timeline({ delay:0.15 });
        tl.to(l1, { y:0, opacity:1, duration:.8, ease:'power3.out' }, 0)
          .to(l2, { y:0, opacity:1, duration:.8, ease:'power3.out' }, 0.25)
          .to(cta, { opacity:1, y:0, duration:.6, ease:'power3.out' }, 0.7);
      }

    });

    /* Subtle cinematic video parallax — the video drifts and scales gently as the
       hero scrolls past, independent of breakpoint since it has no clipping risk. */
    gsap.to('#hero-video', {
      scale:1.15, yPercent:6, ease:'none',
      scrollTrigger:{ trigger:'#hero', start:'top top', end:'bottom top', scrub:0.6 }
    });

    /* Failsafe: the CTA is the primary "contact me" button, so it must never stay
       stuck invisible if this scroll-linked timeline doesn't fire correctly on some
       device/browser — force it visible after a short delay regardless. */
    setTimeout(()=>{
  ...
  function initStats(){
    ...
    if (window.ScrollTrigger){
      ScrollTrigger.create({ trigger: wrap, start: 'top 85%', onEnter: play });
    } else {
      const io = new IntersectionObserver(entries=>{
