...
function initServiceModal(){
  ...
  function openServiceModal(id){
    ...
    requestAnimationFrame(()=>{
      const items = svcModalContent.querySelectorAll('.svc-behance-card,.svc-showcase-card,.svc-vibe-card,.svc-gallery-item,.svc-video-item');
  ...
  document.addEventListener('keydown', e=>{
    if(e.key==='Escape'){
      if(document.getElementById('svcVideoLightbox').classList.contains('open')) closeVideoLightbox();
      else if(svcLightbox.classList.contains('open')) closeLightbox();
  ...
  function buildModalHTML(svc){
    ...
    if(svc.modal==='video'){
      const cats = ['All', ...new Set(AI_VIDEOS.map(v=>v.category))];
      return head + `
        <div class="svc-video-filters">${
          cats.map((c,i)=>`<button class="svc-video-filter${i===0?' active':''}" data-cat="${c}">${c}</button>`).join('')
        }</div>
        <div class="svc-video-grid" id="svcVideoGrid">${
          AI_VIDEOS.map(v=>`
            <div class="svc-video-item" data-cat="${v.category}">
              <div class="svc-video-tag">${v.category}</div>
              <video src="${v.src}" poster="${v.poster||''}" muted loop playsinline preload="metadata"></video>
              <div class="svc-video-overlay">
                <span class="svc-video-play"><span class="icon" data-icon="play"></span></span>
                <span class="svc-video-name">${v.title}</span>
              </div>
            </div>`).join('')
        }</div>`;
    }
  ...
  /* ---- AI video grid: hover-preview, category filter, fullscreen player ---- */
  const svcVideoLightbox = document.getElementById('svcVideoLightbox');
  const svcVideoLightboxVideo = document.getElementById('svcVideoLightboxVideo');
  function attachVideoModule(){
    const grid = document.getElementById('svcVideoGrid');
    if(!grid) return;

    /* hover-to-preview on desktop */
    grid.querySelectorAll('.svc-video-item').forEach(item=>{
      const vid = item.querySelector('video');
      item.addEventListener('mouseenter', ()=> vid.play().catch(()=>{}));
      item.addEventListener('mouseleave', ()=>{ vid.pause(); vid.currentTime = 0; });
      item.addEventListener('click', ()=>{
        svcVideoLightboxVideo.src = vid.currentSrc || vid.src;
        svcVideoLightboxVideo.muted = false;
        svcVideoLightbox.classList.add('open');
        svcVideoLightboxVideo.play().catch(()=>{});
      });
    });

    /* category filter chips */
    svcModalContent.querySelectorAll('.svc-video-filter').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        svcModalContent.querySelectorAll('.svc-video-filter').forEach(b=> b.classList.remove('active'));
  ...
  function closeVideoLightbox(){
    svcVideoLightbox.classList.remove('open');
    svcVideoLightboxVideo.pause();
    svcVideoLightboxVideo.removeAttribute('src');
    svcVideoLightboxVideo.load();
  }
  document.getElementById('svcVideoLightboxClose').addEventListener('click', closeVideoLightbox);
  svcVideoLightbox.addEventListener('click', e=>{ if(e.target===svcVideoLightbox) closeVideoLightbox(); });
