  ...
  SERVICES.forEach((s,i)=>{
    ...
    card.innerHTML = `
      <div class="svc-card-inner">
        ...
        <div class="svc-count-badge"><span class="icon" data-icon="layers"></span>${s.count}</div>
...
/* ============ HERO BRAND ORBIT (floating client/brand logos) ============ */
function initHeroOrbit(){
  const orbitEl = document.getElementById('heroOrbit');
  if(!orbitEl || typeof LOGOS === 'undefined' || !LOGOS.length) return;

  /* Trimmed down to a handful of brand logos (was 15 project thumbs — too busy). */
  const brandLogos = LOGOS.slice(0, 8);
  ...
  const nodes = NODES.map((cfg, i) => {
    ...
    const el = document.createElement('div');
    el.className = 'hero-orbit-node hero-orbit-node--logo';
    el.style.top = cfg.top + '%';
    el.style.left = cfg.left + '%';
    el.style.width = cfg.size;
    el.style.height = cfg.size;
    el.innerHTML = '<img src="' + logo + '" alt="Brand logo" loading="lazy">';
...
/* ============ HERO ROTATING ROLE WORD ("Brand & [UI/UX → Website → Graphic → Product] Designer") ============ */
function initHeroRotatingWord(){
  const el = document.getElementById('heroRotateWord');
  if(!el) return;

  const words = ['UI/UX', 'Website', 'Graphic', 'Product'];
  let i = 0;

  const cycle = () => {
    const tl = gsap.timeline();
    tl.to(el, { yPercent: -60, opacity: 0, filter: 'blur(6px)', duration: .45, ease: 'power2.in' })
      .call(() => {
        i = (i + 1) % words.length;
        el.textContent = words[i];
      })
      .set(el, { yPercent: 60 })
      .to(el, { yPercent: 0, opacity: 1, filter: 'blur(0px)', duration: .5, ease: 'power2.out' });
  };
