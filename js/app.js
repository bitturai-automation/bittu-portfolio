...
(function(){
  ...
  window.addEventListener("load", () => {
    ...
    gsap.to(fill, { width: "100%", duration: 1.1, ease: "power2.out" });
    gsap.to("#loader", {
      opacity: 0, duration: .6, delay: 1.1, ease: "power2.out",
  ...
  /* ---------------- Photo viewer (simple cards: image + close only) ---------------- */
  ...
  document.getElementById("nav-burger").addEventListener("click", () => {
    ...
    links.style.cssText += isOpen ? "" : "position:fixed;top:80px;left:20px;right:20px;background:var(--white);flex-direction:column;padding:20px;border-radius:20px;box-shadow:var(--shadow-card);gap:16px;z-index:950;";
  });
